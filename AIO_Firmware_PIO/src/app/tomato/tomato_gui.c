...
void tomato_gui_init(void) // style init
{
    ...
    lv_style_init(&default_style); // Background
    lv_style_set_bg_color(&default_style, lv_color_hex(0x000000));
    lv_obj_add_style(tomato_scr, &default_style, LV_STATE_DEFAULT);

    lv_style_init(&chFont_style); // Chinese characters
    ...
    lv_style_init(&numberSmall_style); // Small numbers
    ...
    lv_style_init(&numberBig_style); // Large numbers
...
{
    if (tomato_scr == lv_scr_act())
        return; // If it is already the current page, exit
    else
        lv_obj_clean(lv_scr_act()); // Otherwise, clear the page
    if (tomato_scr == NULL)
    {
        tomato_scr = lv_obj_create(NULL);
    }
    ...
    lv_label_set_text(txtLabel, "Focusing");
    lv_obj_set_pos(txtLabel, 31, 165);

    clockLabel_1 = lv_label_create(tomato_scr); // Minutes
    ...
    clockLabel_2 = lv_label_create(tomato_scr); // Seconds
...
void display_tomato(struct TimeStr t, int mode)
{
    ...
    if (t.second == 60) // If seconds are 60, display as 1 minute 00 seconds
    {
        second = 0;
        minute = t.minute + 1;
    }
    ...
    if (mode == 0 || mode == 1)
    {
        if (t.second >= 0 && t.second <= 59)
        {
            ...
            snprintf(labelText, sizeof(labelText), "Focusing%.*s", dots, ".....");
    ...
    else
    {
        if (t.second >= 0 && t.second <= 59)
        {
            char labelText[15];
            int dots = (60 - t.second - 1) % 5 + 1;
            snprintf(labelText, sizeof(labelText), "Resting%.*s", dots, ".....");
            lv_label_set_text(txtLabel, labelText);
        }
    }
    if (second == 0 && minute == 0)
    {
        lv_label_set_text_fmt(txtLabel, "#F5F5F5 %s#", "Time's up!");
    }
}
